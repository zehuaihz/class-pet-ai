...
export default function DashboardPage() {
  ...
  const loadDashboard = useCallback(async (classroomId: string | null) => {
    ...
    try {
      const dashboardData = await apiRequest<DashboardPayload>(`/api/v1/classrooms/${classroomId}/dashboard`)
  ...
  useEffect(() => {
    async function load() {
      try {
        const classroomData = await apiRequest<{ items: ClassroomOption[] }>("/api/v1/classrooms")
        const classroomItems = classroomData.items
        ...
        <section className="grid gap-6 lg:grid-cols-2">
          <div className="rounded-xl bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              {selectedClassroomId ? <>
                <Link href={`/classrooms/${selectedClassroomId}/students`} className="rounded-lg border px-4 py-2">学生管理</Link>
                <Link href={`/classrooms/${selectedClassroomId}/points`} className="rounded-lg border px-4 py-2">积分管理</Link>
              </> : <span className="rounded-lg border border-dashed px-4 py-2 text-slate-500">请先创建班级</span>}
            </div>
          </div>
          <div className="rounded-xl bg-white p-5 shadow-sm">
