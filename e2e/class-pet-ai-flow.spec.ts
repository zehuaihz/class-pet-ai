...
test.describe("class pet AI teacher flows", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/api/v1/classrooms/class_1/point-rules", async (route) => {
      await route.fulfill({ json: { success: true, data: { items: [{ id: "rule_1", name: "作业优秀", pointDelta: 2 }] }, error: null, meta: null } })
    })
    ...
    await page.route("**/api/v1/ai/comment-draft", async (route) => {
      ...
      await route.fulfill({ json: { success: true, data: { jobId: "job_1" }, error: null, meta: null } })
    })

    await page.route("**/api/v1/ai/jobs/job_1", async (route) => {
      await route.fulfill({ json: { success: true, data: { status: "SUCCEEDED", outputJson: { text: "小明本周课堂表现积极，阅读打卡稳定。" } }, error: null, meta: null } })
    })
  })
